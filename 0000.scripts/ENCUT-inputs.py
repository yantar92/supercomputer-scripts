#!/usr/bin/env python3
# [[file:/home/yantar92/Org/notes.org::*ENCUT convergence script][ENCUT convergence script:1]]
...
def generate_inputs(args) -> None:
    ...
    for encut in np.arange(args.encut_min, args.encut_max, args.encut_step):
        ...
        directory=f"{system_name}.ENCUT.{encut}"
        current_input = vasp_input.copy()

        vc_relax_overrides = {"ENCUT": float(encut)}
...
def main() -> None:
    ...
    The input files will be placed into <SYSTEM>.ENCUT.<encut value>
    folders.
    """,
...
if __name__ == "__main__":
    main()
# ENCUT convergence script:1 ends here
