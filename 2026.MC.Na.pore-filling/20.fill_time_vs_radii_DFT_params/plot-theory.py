...
for radius in radii:
    ...
    max_idx = 100
    idx = 0
    for f in sorted(chain(Path().glob(f"fix_samples_0V/{VOLTAGE}V_{TEMPERATURE}K_{radius}A_r*.csv.gz"),
                          Path().glob(f"large_pore/{VOLTAGE}V_{TEMPERATURE}K_{radius}A_r*.csv.gz"))):
        idx += 1
        if idx > max_idx:
            continue
        print(f)
...
fig.savefig(f"theory-{TEMPERATURE}K-{VOLTAGE}V.png", bbox_inches="tight")
fig.savefig(f"theory-{TEMPERATURE}K-{VOLTAGE}V.svg", bbox_inches="tight")
