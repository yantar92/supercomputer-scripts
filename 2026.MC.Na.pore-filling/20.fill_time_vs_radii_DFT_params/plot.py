...
for idx, radius in enumerate(reversed(radii)):
    ...
    ax.plot(df_median.index/100, df_median["Filling"], label=f"$d$ = {true_radius*2/10:.1f} nm", linewidth=1, color=cmap(idx))
...
fig.savefig(f"Figure4b-{TEMPERATURE}K-{VOLTAGE}V_{SAMPLES}.png", bbox_inches="tight")
fig.savefig(f"Figure4b-{TEMPERATURE}K-{VOLTAGE}V_{SAMPLES}.svg", bbox_inches="tight")
plt.show()
