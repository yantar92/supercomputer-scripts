...
def get_entries_recursively(
    ...
    tem = IMDGVaspDir.read_vaspdirs(path, path_filter=lambda p: ('PBE+D3-BJ' == p.name and 'FAILED' not in str(p)))
    # tem = IMDGVaspDir.read_vaspdirs(path, path_filter=lambda p: (('relax.2' == p.name) or ('relax.final.optB88-vdw' == p.name)) and 'FAILED' not in str(p))
    # tem = IMDGVaspDir.read_vaspdirs(path, path_filter=lambda p: 'relax.2' == p.name and 'FAILED' not in str(p))
    # tem = IMDGVaspDir.read_vaspdirs(path, path_filter=lambda p: 'relax.2.optB88-vdW' == p.name and 'FAILED' not in str(p))
    # tem = IMDGVaspDir.read_vaspdirs(path, path_filter=lambda p: 'relax.final' == p.name and 'FAILED' not in str(p))
    # tem = IMDGVaspDir.read_vaspdirs(path, path_filter=lambda p: ('relax.final.optB88-vdW' == p.name or 'relax.2.optB88-vdW' == p.name) and 'FAILED' not in str(p))
    vasp_dirs = [Path(p) for p in tem]
    del tem
