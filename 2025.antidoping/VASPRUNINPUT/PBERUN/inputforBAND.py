...
for subfolder in os.listdir(main_dir):
    ...
    for sub_subfolder in os.listdir(subfolder_path):
        ...
        if os.path.exists(Relax_folder):
            # Create BAND folder if it does not exist
            if not os.path.exists(BAND_folder):
                try:
                    os.mkdir(BAND_folder)
                    print(f"Created BAND folder: {BAND_folder}")
                except Exception as e:
                    print(f"Failed to create BAND folder {BAND_folder}: {e}")
            ...
            dst_poscar = os.path.join(BAND_folder, 'POSCAR')     # Destination POSCAR
            
            # Copy CONTCAR to BAND/POSCAR if CONTCAR exists
            ...
            poscar_file = dst_poscar  # BAND/POSCAR
            
            # Check if POSCAR exists in BAND folder
            if os.path.exists(poscar_file):
                try:
                    ...
                    print(f"INCAR and KPOINTS have been generated for {BAND_folder}")
                except Exception as e:
