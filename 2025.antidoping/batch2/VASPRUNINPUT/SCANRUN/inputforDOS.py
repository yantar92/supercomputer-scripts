    ...
    for sub_subfolder in os.listdir(subfolder_path):
        ...
        if os.path.isdir(sub_subfolder_path):  # Check if it's a directory
            ...
            if os.path.exists(Relax_folder):
                # Create DOS_folder if it does not exist
                if not os.path.exists(DOS_folder):
                    os.mkdir(DOS_folder)
                    print(f"Created DOS folder at {DOS_folder}")
                
                src_contcar = os.path.join(Relax_folder, 'CONTCAR')  # Source CONTCAR
                dst_poscar = os.path.join(DOS_folder, 'POSCAR')      # Destination POSCAR in DOS folder
                
                # Copy CONTCAR to POSCAR within DOS folder if CONTCAR exists
                ...
                poscar_file = dst_poscar  # POSCAR in DOS_folder
                
                if os.path.exists(poscar_file):  # Check if POSCAR file exists in the DOS folder
                    ...
                    # Define file paths for INCAR and KPOINTS within DOS folder
                    ...
                    # shutil.copy('path_to_POTCAR', DOS_folder)
        
                    print(f"INCAR and KPOINTS have been generated for {DOS_folder} using SCAN functional.")
                else:
