...
find . -mindepth 2 -maxdepth 2 -type d | while read d; do
	...
	(cd $d/$functional_path;
	 ~/data/2025.graphite-expanded.CE/atat-formation-energies.py --format=${FORMAT} --ymin=${ymin} --ymax=${ymax} --title="${ion}-C ($(dirname $d) stacking) phase diagram. d002=relax; $functional"  --max_composition="${ion}C3" --ion ${ion} $me_path ./0/ATAT.SCF --extra_data perturb.*
	)
    done
done
