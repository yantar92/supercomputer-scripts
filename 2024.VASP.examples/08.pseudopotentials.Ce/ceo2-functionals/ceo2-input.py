...
class CeO2(VaspInputSet):
    ...
    CONFIG = {'INCAR':
              ...
              'POTCAR': {'Ce': 'Ce'+ase_potential_defaults['recommended']['Ce'] if 'Ce' in ase_potential_defaults['recommended'] else "",
                         'O': 'Os'+ase_potential_defaults['recommended']['Os'] if 'Os' in ase_potential_defaults['recommended'] else ""}
              }
    def __post_init__(self) -> None:
        with MPRester() as m:
            structure = m.get_structure_by_material_id("mp-20194")  # CeO2
...
parser = argparse.ArgumentParser(
    description="""
    Generate CeO2 inputs, possibly specifying potentials.
...
inputset = CeO2(user_potcar_settings=args.potentials)
